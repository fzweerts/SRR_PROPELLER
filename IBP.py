...
while iteration < max_iter:
    ...
    G = np.zeros_like(X, dtype=np.complex64)
    ...
    for n in range(N):
        ...
        # Acuumulate the gradient for all blades
        G += g_n
    ...
    denominator = np.real(np.sum(np.conj(G) * EHEg)) #use np.real to make sure output for lambda is real (not complex), np.conj is needed
